...
`config/scenarios/`. Der Name ist der Dateistamm und verbindet beides:

    scenarien/lane_departure_3_lanes.mp4     die Aufnahme
    config/scenarios/lane_departure_3_lanes.yaml   nur die Abweichungen
...
class Scenario:
    ...
    config_overlay: Path | None       # config/scenarios/<name>.yaml, falls vorhanden
    ...
    def describe(self, ground_truth_dir: str | Path = "ground_truth") -> str:
        ...
        annotated = (f"{len(truth.events)} Sollereignis(se)" if truth is not None
                     else "keine Annotation")
        return f"{self.name:<32s} {kind:<7s} {overlay:<22s} {annotated}"


def discover(scenario_dir: Path = DEFAULT_SCENARIO_DIR,
             config_dir: Path = Path("config")) -> list[Scenario]:
